    ...
    <tr>
        ...
        <td class="text-center">
            <form class="d-flex" action="{{ route('m_user.destroy', $m_user->user_id) }}" method="POST">
                ...
                <a class="btn btn-primary btn-sm mr-2 m-1" href="{{ route('m_user.edit', $m_user->user_id) }}">Edit</a>
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm mr-2 m-1" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">Delete</button>
            </form>
        </td>
    </tr>
    @endforeach
